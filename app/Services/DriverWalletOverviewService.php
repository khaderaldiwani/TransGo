...
class DriverWalletOverviewService
{
    public function getWalletOverview(User $actor): array
    {
        $wallet = $this->resolveDriverWallet($actor);
    ...
    private function transformTransactionCard(WalletTransaction $transaction): array
    {
        ...
        return [
            ...
            'details_endpoint' => '/api/v1/driver/wallet/transactions/'.$transaction->transaction_id,
    ...
    private function resolveTitle(WalletTransaction $transaction, ?Receipt $receipt): string
    {
        return match ($receipt?->receipt_type) {
            ...
            'booking_income' => 'دخل حجز إلكتروني',
            'booking_rejection_reversal' => 'استرداد بعد رفض الحجز',
            'trip_cancellation_reversal' => 'استرداد بعد إلغاء الرحلة',
            'booking_refund_reversal' => 'استرداد بعد إلغاء الحجز',
    ...
    private function resolveDriverWallet(User $actor): Wallet
    {
        $this->resolveDriverProfile($actor);
